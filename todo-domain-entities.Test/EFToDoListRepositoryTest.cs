...
namespace todo_domain_entities.Test
{
    ...
    public class EFToDoListRepositoryTest : IDisposable
    {
        ...
        EFToDoListRepository CreateEFToDoListRepository()
            => new EFToDoListRepository(new AppDbContext(_contextOptions));
        ...
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();

            // Act and assert
            Assert.Throws<ArgumentException>(() => efRepository.AddToDoList(toDoList));
        }

        [TestCase, Order(2)]
        public void AddToDoList_PassedValidObject_ReturnsInsertedInstance()
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            ...
            // Act
            var resultItem = efRepository.AddToDoList(toDoList);

            // Assert
            Assert.IsTrue(efRepository.ToDoLists.Contains(resultItem));
        }
        ...
        public void AddToDoList_PassedValidArgs_SuccessfulExecution(ToDoList toDoList)
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var todoListItemsCount = efRepository.ToDoLists.Count();

            // Act
            efRepository.AddToDoList(toDoList);

            // Assert
            Assert.AreEqual(todoListItemsCount + 1, efRepository.ToDoLists.Count());
        }
        ...
        public void RemoveToDoList_PassedInvalidArgs_ThrowsArgumentException(ToDoList toDoList)
        {
            ...
            // Act and assert
            Assert.Throws<ArgumentException>(() => efRepository.RemoveToDoList(toDoList));
        }
        ...
        public void RemoveToDoList_PassedValidObject_SuccessfulExecution()
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var existingInstance = efRepository.ToDoLists.First(list => list.PrimaryPurpose == "Become a superhero");
            var todoListItemsCount = efRepository.ToDoLists.Count();

            // Act
            efRepository.RemoveToDoList(existingInstance);
        ...
        public void ModifyToDoList_InvalidArgs_ThrowsArgumentException(ToDoList listToUpdate, ToDoList updatedListView)
        {
            ...
            // Act and assert
            Assert.Throws<ArgumentException>(() => efRepository.ModifyToDoList(listToUpdate, updatedListView));
        }
        ...
        public void ModifyToDoList_ValidArgs_ReturnsUpdatedInstance(int listToUpdateId, ToDoList updatedListView)
        {
            // Arange
            using var efRepository = CreateEFToDoListRepository();
            var listToUpdate = efRepository.ToDoLists.First(list => list.Id == listToUpdateId);
            updatedListView.ToDoEntries.ForEach(entry => entry.ToDoList = updatedListView);

            // Act
            var updatedInstance = efRepository.ModifyToDoList(listToUpdate, updatedListView);
        ...
        public void AddToDoEntriesToList_InvalidArgs_ThrowsArgumentException(List<ToDoEntry> toDoEntries, int toDoListId)
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var toDoList = efRepository.ToDoLists.FirstOrDefault(list => list.Id == toDoListId);

            // Act and assert
            Assert.Throws<ArgumentException>(() => efRepository.AddToDoEntriesToList(toDoEntries, toDoList));
        }
        ...
        public void AddToDoEntriesToList_ValidArgs_SuccessfulExecution(List<ToDoEntry> toDoEntries, int toDoListId)
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var toDoList = efRepository.ToDoLists.First(list => list.Id == toDoListId);
            toDoEntries.ForEach(entry => entry.ToDoList = toDoList);

            // Act
            efRepository.AddToDoEntriesToList(toDoEntries, toDoList);
        ...
        public void ModifyToDoEntry_InvalidArgs_ThrowsArgumentException(ToDoEntry toDoEntryToUpdate, ToDoEntry updatedView)
        {
            ...
            // Act and assert
            Assert.Throws<ArgumentException>(() => efRepository.ModifyToDoEntry(toDoEntryToUpdate, updatedView));
        }
        ...
        public void ModifyToDoEntry_ValidArgs_ReturnsUpdatedInstance(int toDoEntryToUpdateId, ToDoEntry updatedView)
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var toDoEntryToUpdate = efRepository.ToDoLists.SelectMany(list => list.ToDoEntries)
                                                          .First(entry => entry.Id == toDoEntryToUpdateId);
            updatedView.ToDoList = toDoEntryToUpdate.ToDoList;

            // Act
            var updatedInstance = efRepository.ModifyToDoEntry(toDoEntryToUpdate, updatedView);
        ...
        public void RemoveToDoEntry_PassedInvalidArgs_ThrowsArgumentException(ToDoEntry toDoEntry)
        {
            ...
            // Act and assert
            Assert.Throws<ArgumentException>(() => efRepository.RemoveToDoEntry(toDoEntry));
        }
        ...
        public void RemoveToDoEntry_PassedValidObject_SuccessfulExecution()
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var existingInstance = efRepository.ToDoLists.First();
            var todoListItemsCount = existingInstance.ToDoEntries.Count;

            // Act
            efRepository.RemoveToDoEntry(existingInstance.ToDoEntries.Last());
        ...
        public void SetToDoEntryStatus_InvalidArgs_ThrowsArgumentException(ToDoEntry toDoEntryToUpdate, ToDoStatus status)
        {
            ...
            // Act and assert
            Assert.Throws<ArgumentException>(() => efRepository.SetToDoEntryStatus(toDoEntryToUpdate, status));
        }
        ...
        public void SetToDoEntryStatus_ValidArgs_ReturnsUpdatedInstance(int todoListId, int toDoEntryToUpdateId, ToDoStatus status)
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var todoList = efRepository.ToDoLists.Single(list => list.Id == todoListId);
            var toDoEntryToUpdate = todoList.ToDoEntries.Single(entry => entry.Id == toDoEntryToUpdateId);

            // Act
            var updatedInstance = efRepository.SetToDoEntryStatus(toDoEntryToUpdate, status);
        ...
        public void ClearToDoList_NoArgs_ReturnsUpdatedToDoListItem()
        {
            // Arrange
            using var efRepository = CreateEFToDoListRepository();
            var toDoList = efRepository.ToDoLists.First(list => list.Id == 3);

            // Act
            var listWithoutToDoItems = efRepository.ClearToDoList(toDoList);
        ...
        public void RemoveAllToDoLists_NoArgs_SuccessfulExecution()
        {
            ...
            // Act
            efRepository.RemoveAllToDoLists();

            // Assert
            Assert.IsTrue(efRepository.ToDoLists.Count() == 0);
        }
